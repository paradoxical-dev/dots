return {
	{
		...
		opts = function(_, opts)
			...
			local colors = {
				...
				bright_fg = "#D0D0D1",
				dim_fg = utils.get_highlight("Folded").fg,

				normal = utils.get_highlight("HeirlineNormal").bg,
				...
				git_bg = utils.get_highlight("RenderMarkdownH4Bg").bg,
				git_fg = utils.get_highlight("RenderMarkdownH4Bg").fg,
			...
			local ViMode = {
				...
				hl = function(self)
					...
					return { fg = self.mode_colors[mode], bg = self.mode_colors[mode], bold = true }
				end,
				update = {
					"ModeChanged",
					pattern = "*:*",
